...
// lockIssueTopicSlot serializes issue-topic creation for a single issue across
// every entrypoint — the project-route sync worker and the direct-route service
// binds — using a transaction-scoped Postgres advisory lock keyed by
// (workspace_id, issue_id).
//
// Callers MUST pass a transaction. pg_advisory_xact_lock releases at
// commit/rollback; on the bare pool it would attach to a throwaway connection
// and unlock immediately, serializing nothing. Held across the
// "no active topic yet? -> create Lark root -> insert binding" sequence, it
// guarantees at most one Lark root message and one active binding per issue even
// when a stale lease causes the same outbox item to be reprocessed after Lark's
// idempotency window has closed, or when a direct bind races the project route.
func (s *projectSyncStore) lockIssueTopicSlot(ctx context.Context, tx pgx.Tx, workspaceID, issueID pgtype.UUID) error {
	_, err := tx.Exec(ctx,
...
func (s *projectSyncStore) getCurrentProjectBindingForUpdate(ctx context.Context, q projectSyncDB, workspaceID, projectID pgtype.UUID) (ChannelProjectBinding, error) {
	return scanProjectBinding(q.QueryRow(ctx, `
		SELECT `+projectBindingColumns+`
		FROM channel_project_binding
		WHERE workspace_id = $1 AND project_id = $2
		...
		ORDER BY created_at DESC
		LIMIT 1
		FOR UPDATE`, workspaceID, projectID))
}

func (s *projectSyncStore) rotatePendingProjectBinding(ctx context.Context, q projectSyncDB, binding ChannelProjectBinding, tokenHash string, expiresAt time.Time) (ChannelProjectBinding, error) {
	return scanProjectBinding(q.QueryRow(ctx, `
		UPDATE channel_project_binding
		SET bind_token_hash = $1, bind_token_expires_at = $2, updated_at = now()
		WHERE id = $3 AND workspace_id = $4 AND installation_id = $5
		  AND state = 'pending_group'
		RETURNING `+projectBindingColumns,
		tokenHash, expiresAt, binding.ID, binding.WorkspaceID, binding.InstallationID,
	))
}
...
func (s *projectSyncStore) deadNotification(ctx context.Context, item ChannelNotificationOutbox, workerID, lastError string) error {
	tag, err := s.pool.Exec(ctx, `
		UPDATE channel_notification_outbox
		SET status = 'dead', attempts = attempts + 1, last_error = $1,
		    locked_at = NULL, locked_by = NULL
		WHERE id = $2 AND status = 'sending' AND locked_by = $3`, lastError, item.ID, workerID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
