...
func (w *ProjectIssueSyncWorker) ensureIssueTopic(ctx context.Context, item ChannelNotificationOutbox, projectBinding ChannelProjectBinding, issue db.Issue, payload projectNotificationPayload, creds InstallationCredentials) (ChannelIssueTopicBinding, bool, error) {
	...
	// Render BEFORE opening the transaction. renderIssueCreated reads the
	// workspace prefix and project title through w.sync.queries, which is bound
	// to the bare pool, not to tx. Calling it while this goroutine already holds
	// a pooled connection for tx deadlocks the whole slow path: with N concurrent
	// creators and a pool of N or fewer, every connection is held by a waiting
	// transaction, so the one goroutine that won the advisory lock can never
	// acquire the second connection it needs to finish — and the losers wait on a
	// lock that is never released. The rendered text is presentation only and has
	// no correctness dependency on the locked section, so reading it earlier is
	// safe; the only cost is rendering text we discard when the re-check below
	// finds another creator already won.
